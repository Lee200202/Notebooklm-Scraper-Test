...
>
> 🔄 **自動切換機制說明**：
> - 轉錄時只要金鑰**第一次失敗**（負載過高、429、400、狀態 failed、逾時…），**不等待，馬上換下一組金鑰**重送。
> - 這一輪所有金鑰都失敗，才等 30 秒（之後 90 秒）再從第一組輪一次，最多 3 輪。
> - 連續兩輪所有金鑰都回 429（或訊息是每日額度），才判定額度用完，改用備援模型（若有設定）。
> - **金鑰快速檢查**：送影片前，先用一個極小的文字請求確認這組金鑰目前可用，幾秒內就會回應。
>   額度用完、模型負載過高（high demand）、金鑰無效或逾時，都會**不送影片、馬上跳下一組**，不用等轉錄 1～2 分鐘後才失敗。
>   通過檢查或剛轉錄成功的金鑰，5 分鐘內不重複檢查；轉錄失敗的金鑰下次使用前會重新檢查。
>   每次檢查會用掉 1 次請求額度。小請求通過不代表長影片一定成功，轉錄途中失敗時仍會馬上換金鑰。
> - **串流中斷自動接續**：串流模式（例如 gemini-3.5-flash-lite）轉錄途中連線被切斷時，會從中斷處接續接收，已收到的文字保留、不重送影片；接續 3 次仍失敗才換金鑰。
...
| `KEY_PROBE` | `true` | 送影片前先快速檢查金鑰是否可用；設 `false` 關閉 |
| `KEY_PROBE_TIMEOUT_SECONDS` | `20` | 快速檢查幾秒沒回應就視為不可用 |
| `KEY_PROBE_TTL_SECONDS` | `300` | 金鑰確認可用後，幾秒內不重複檢查 |
...
| 服務 | 用量 | 費用 |
|---|---|---|
| Gemini API（免費方案） | 每集約 3 次請求（每 30 分鐘一段） | gemini-3.5-flash-lite 支援多組 API 金鑰輪替（第一次失敗就馬上換 Key） |
...
- **額度按「專案＋模型」計算**：若需多份獨立額度，金鑰需建在不同 GCP 專案或 Google 帳號下。本專案支援設定多組 API 金鑰（如 `GEMINI_API_KEY_2`、`GEMINI_API_KEYS`），任何一次失敗都會馬上換下一組金鑰繼續。
...
有多組金鑰時，第一次失敗就馬上換下一組；所有金鑰這一輪都失敗才等 30 秒、90 秒再輪，最多 3 輪。都失敗才算這集失敗，之後的排程會再試。

**Q：log 出現「串流連線中斷（… peer closed connection without sending complete message body …）」？**
轉錄時間較長時，伺服器或網路可能中途切斷串流連線。程式會自動從中斷處接續接收，不用處理；接續 3 次仍失敗才會換金鑰。

**Q：log 的時間是哪個時區？**
程式印出的時間是台灣時間。GitHub 介面左側另外顯示的時間戳記則是 UTC。

**Q：log 出現「HTTP 429 … You exceeded your current quota」？**
表示 Gemini 額度超額。程式會**馬上切換到下一組 API 金鑰**繼續轉錄；所有金鑰連續兩輪都回 429 才判定額度用完。若所有金鑰在主要模型皆耗盡，才會切換至備援模型（若有設定）。若所有金鑰與模型額度皆耗盡，才會記錄「配額不足」並停止，等 15:00 或 16:00 額度重置後的排程補跑。
「模型」欄會記錄這集實際用了哪些模型。
