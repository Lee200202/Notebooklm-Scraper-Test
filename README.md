...
1. **這個 repo 是公開的**：任何人都看得到程式碼和 Actions 的執行 log。
   - 程式**不會**把金鑰或逐字稿內容印到 log。服務帳戶 email 等資訊也會自動遮蔽成 `***`。
   - 逐字稿只存在**你自己的私人試算表**，不會放進 repo。
2. **60 天規則**：公開 repo 如果連續 60 天沒有任何活動（例如 commit），GitHub 會自動停用排程。
   停用後到 Actions 頁面按「Enable workflow」就能恢復。或者把 repo 改成 Private，就沒有這個限制，但每月只有 2,000 分鐘免費額度（本專案每月約用 700～900 分鐘）。
